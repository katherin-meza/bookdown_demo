...
raw_dara_1980<-GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/1980")
unpack_data_1980<- httr::content(raw_dara_1980, as= "text", encoding = "UTF-8")
final_data_1980<- jsonlite::fromJSON(unpack_data_1980)
final_data_1980

raw_data_1999<-GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/1999")
unpack_data_1999<-httr::content(raw_data_1999, as="text", encoding = "UTF-8")
final_data_1999<-jsonlite::fromJSON(unpack_data_1999)
final_data_1999      

raw_data_2018<-GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/2018")
unpack_data_2018<-httr::content(raw_data_2018, as="text", encoding = "UTF-8")
final_data_2018<-jsonlite::fromJSON(unpack_data_2018)
final_data_2018
...
raw_data_1<- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startYear=2010&endYear=2021")

unpacked_data <- httr::content(raw_data_1, as = "text", encoding = "UTF-8") 
...
```{r}
#Downloading data to know all the code for parks and saved in our file
all_park_code<- GET(url = "https://www.nps.gov/aboutus/foia/upload/NPS-Unit-List.xlsx")
unpaking_all_code<- content(all_park_code, as=  "raw")
writeBin(unpaking_all_code, "NPS-Unit-List.xlsx")

#From our previous link we are going to pull and to have a new data frame similar as what we will build with the function

get_data<- GET(url = "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=YELL&startMonth=1&startYear=2015&endMonth=12&endYear=2016")

extracted_data_example <- httr::content(get_data, as = "text", encoding = "UTF-8") 

final_data_all <- jsonlite::fromJSON(extracted_data_example )
  final_data_all

#Creating a unit_visitation function
year_start<- (2015)
year_end<-(2017)
month_start<- (1)
month_end<-(12)
...
#this is the format of the complex link ===  https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes={UNITCODES}&startMonth={STARTMONTH}&startYear={STARTYEAR}&endMonth={ENDMONTH}&endYear={ENDYEAR}

unit_visitation<- function(year_start,year_end, month_start,month_end,id_name){
  get_data_year<- GET(url =paste0( "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=", id_name, "&startMonth=",  month_start, "&startYear=", year_start, "&endMonth=", month_end, "&endYear=", year_end))
...
test<-unit_visitation(year_start,year_end, month_start,month_end,id_name)
test

test<-unit_visitation(year_start=2016,year_end=2017, month_start=2,month_end=6,id_name="YELL")
test

#what's the difference between `05` and `"05"`?

##`05` is a structure that can be recognized as integer, While "05" is a character data type. 
...
```{r}
#Creating the object and the function
...
test_ROMO<-unit_visitation(year_start,year_end, month_start,month_end,id_name="ROMO")
head(test_ROMO)  
test_EVER<-unit_visitation(year_start,year_end, month_start,month_end,id_name= "EVER")
head(test_ROMO)
test_THRO<-unit_visitation(year_start,year_end, month_start,month_end,id_name="THRO")
head(test_ROMO)
...
#HERE: Replacing any code name in id_name will obtain a different data-frames for each park.

#For "ROMO"
unit_visitation<- function(year_start="1980",year_end="2021", month_start="1",month_end="12",id_name="ROMO"){
  get_data_year<- GET(url =paste0( "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=", id_name, "&startMonth=",  month_start, "&startYear=", year_start, "&endMonth=", month_end, "&endYear=", year_end))
extracted_data_yell <- httr::content( get_data_year, as = "text", encoding = "UTF-8") 
...
#For "EVER"
unit_visitation<- function(year_start="1980",year_end="2021", month_start="1",month_end="12",id_name="EVER"){
  get_data_year<- GET(url =paste0( "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=", id_name, "&startMonth=",  month_start, "&startYear=", year_start, "&endMonth=", month_end, "&endYear=", year_end))
extracted_data_yell <- httr::content( get_data_year, as = "text", encoding = "UTF-8") 
...
unit_visitation<- function(year_start="1980",year_end="2021", month_start="1",month_end="12",id_name="THRO"){
  get_data_year<- GET(url =paste0( "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=", id_name, "&startMonth=",  month_start, "&startYear=", year_start, "&endMonth=", month_end, "&endYear=", year_end))
extracted_data_yell <- httr::content( get_data_year, as = "text", encoding = "UTF-8") 
...
various_ids<- c("ROMO", "EVER", "THRO")
id_output_floop <- vector("list", length = length(various_ids))

for(i in 1:length(various_ids)){
id_output_floop[[i]]<-
unit_visitation(id_name = various_ids[i])

}

summary(id_output_floop)

#Binding frames
multi_ids_names <- dplyr::bind_rows(id_output_floop)
multi_ids_names
```
...
ITERATION FUNCTION == map() in purrr and tidyverse
...
#Based on our previous unit_visitation()

various_ids<- c("ROMO", "EVER", "THRO")

output_map_unit <- various_ids %>% 
  map(~ unit_visitation(id_name = .))

multi_ids <- bind_rows(output_map_unit)
multi_ids
